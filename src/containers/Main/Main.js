...
import { tfl_app_id, tfl_app_key } from '../../assets/keys';
...
const Main = ({ data }) => {
    ...
    const submitHandler = async (values) => {
        ...
        try {
            ...
            dispatch('ADD_DATA', {
                ...
                meta: {
                    ...
                    from: {
                        ...
                        coordinates: {
                            lat: parseFloat(response.data.journeyVector.from.split(':')[1]),
                            lng: parseFloat(response.data.journeyVector.from.split(':')[0])
                        }
                    },
                    to: {
                        name: values.destination.name,
                        coordinates: {
                            lat: parseFloat(response.data.journeyVector.to.split(':')[1]),
                            lng: parseFloat(response.data.journeyVector.to.split(':')[0])
                        }
                    }
                }
            });
            ...
            let message = error.response && error.response.status === 300 ? 'Travelling outside London?' : 'Please, try again.';
            ...
            },
            {
                ...
                fields: [
                    { 
                        name: 'timeIs',
                        options: [
                            { name: 'departing', value: 'departing' },
