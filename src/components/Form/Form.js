    ...
    const isSubmitEnabled = () => {
        for(let section of data) {
            for(let field of section.fields) {
                if(field.required && (values == null || values[field.name] == null || values[field.name] === '' || values[field.name].name === '' || values[field.name].name === null)) {
                    return false;
                }
            }
        }
                        ...
                        case 'location':
                            component =
                                <Location                            
                                    ...
                                    onChange = { (name, coordinates) => setValues({ ...values, [field.name]: { name: name, coordinates: coordinates } }) }
                                    detectCurrentLocation = { field.detectCurrentLocation }
                                    key = { field.name } />
                            break;
